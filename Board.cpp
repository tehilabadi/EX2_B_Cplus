...
namespace ariel{
    ...
    string Board::read(unsigned int raw, unsigned int cul, Direction Dir, unsigned int numOfChar ){
        if(numOfChar==0){
            return "";
        }
        string ans;
    ...
    void Board::show(){
        for (unsigned int i = maxLeft; i <= minRight+2; i++){
            cout<<'_';
        }
        cout<<"\n";
        for (unsigned int i = maxTop; i <=minBottom+1; i++){
                cout<<'_';        
